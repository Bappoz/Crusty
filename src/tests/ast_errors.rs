...
mod test {
    ...
    fn test_nested_pointer_types() {
        ...
        if let Type::Pointer(inner) = q_type.ty {
            // Verificação se q_type.ty é enum Pointer e depois passa para inner
            if let Type::Pointer(base) = *inner {
                // Verifica se o q_type.ty está apontando para outro Pointer
    ...
    fn test_unary_operations() {
        ...
        let expr = Box::new(Expr::Ident(
            "IdentificadoVariável".to_string(),
            span.clone(),
        ));
